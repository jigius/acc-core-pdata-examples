...
use DomainException, DateTimeImmutable, DateTimeZone;

/**
 * Class Update
 * @package Acc\Core\PersistentData\Examples\PDO\Request
 */
final class Update implements RequestInterface
{
    /**
     * @var EntityInterface
     */
    ...
    public function executed(ExtendedPDOInterface $pdo): RequestInterface
    {
        ...
        try {
            $obj = $this->withAttr("type", "updated");
            ...
            if (!$this->i->positions()->fetch('attrs')->orig()->value('persisted', false)) {
                throw new DomainException("`update` operation is prohibited for the entity");
            }
            $obj
                ->statement =
                $pdo
            ...
            $obj =
                $obj
                    ...
                    ->withAttr(
                        'id',
                        $this->i->positions()->fetch('id')->orig()
                    );
    ...
    {
        $p = $this->i->positions();
        if (!$p->fetch('attrs')->orig()->value('dirty', false)) {
            return "";
        }
        $chunks = [
            "UPDATE `foo`",
